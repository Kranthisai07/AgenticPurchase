...
Test Harness
- Runner: `scripts/run_eval.py` executes batch jobs over a YAML dataset, calling `/saga/preview` (S1–S4) or `/saga/start` (S1–S5) and writing JSONL to `Agentic_AI/logs/eval.log`.
...
Reproducibility
- Start server: `python -m uvicorn Agentic_AI.apps.coordinator.main:app --reload` (or `Agentic_AI.langserve_app:app`).
...
Output Artifacts
- `Agentic_AI/logs/eval_summary.csv` — per-run metrics (recognition_hit, top1_brand_hit, top3_brand_hit, dt_wall_s, lat_*).
- `Agentic_AI/logs/stage_latency.csv` — per-run per-stage dt_s.
- `Agentic_AI/logs/token_summary.csv` — TOKEN events (if present) for prompt/completion usage by stage.
